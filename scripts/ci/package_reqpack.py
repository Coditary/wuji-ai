...
def build_metadata(
    ...
    return {
        ...
        "release": 1,
        "revision": 0,
        "summary": "Unified AI CLI",
        "description": "wuji — CLI frontend for AI workloads via the Wuji core daemon.",
        "license": "MIT",
        "architecture": arch,
...
def copy_docs(payload_doc_dir: Path, repo_root: Path) -> None:
    ...
    if readme.is_file():
        shutil.copy2(readme, payload_doc_dir / "README.md")


def main() -> int:
    parser = argparse.ArgumentParser(description="Package wuji as ReqPack .rqp archive")
    ...
    with tempfile.TemporaryDirectory(prefix=f"wuji-rqp-{args.platform}-{args.arch}-", dir=output_dir) as temp_dir:
